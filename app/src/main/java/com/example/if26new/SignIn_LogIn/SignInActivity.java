...
public class SignInActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        GradientDrawable gd = new GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM, new int[] {Color.parseColor(getResources().getString(R.string.startColor)),Color.parseColor(getResources().getString(R.string.endColor))});
    ...
    public void showPopUp(){
        eraseAccount.setContentView(R.layout.pop_up_erase_account);
        ...
        confirmEraseAccount.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //For the recognition playlist
                PlaylistModel playlistUserRecognition = new PlaylistModel(db.userDao().getUserFromUsername(username.getText().toString()).getId(),"Recognition",R.drawable.musicwave);
                db.mPlaylistDao().insertPlaylist(playlistUserRecognition);
                Intent mainLayout = new Intent(SignInActivity.this, MainActivity.class);
                startActivity(mainLayout);
                    ...
                    if (fingerPrintSwitch.isChecked()){
                        ...
                        if(isAlreadyAccountWithFingerPrint==true){
                           showPopUp();
                            ...
                            userModel=new UserModel(username.getText().toString(),password.getText().toString(),mailAddress.getText().toString(),R.drawable.default_avatar, Color.parseColor(getResources().getString(R.string.startColor)),Color.parseColor(getResources().getString(R.string.endColor)),Color.parseColor("#4A86E8"),true);
                            db.userDao().createUser(userModel);
                            PlaylistModel playlistUser=new PlaylistModel(db.userDao().getUserFromUsername(username.getText().toString()).getId(),"Favorite",R.drawable.like);
                            db.mPlaylistDao().insertPlaylist(playlistUser);
                            //For the recognition playlist
                        ...
                        userModel=new UserModel(username.getText().toString(),password.getText().toString(),mailAddress.getText().toString(),R.drawable.default_avatar, Color.parseColor(getResources().getString(R.string.startColor)),Color.parseColor(getResources().getString(R.string.endColor)),Color.parseColor("#4A86E8"),false);
                        db.userDao().createUser(userModel);
                        PlaylistModel playlistUser=new PlaylistModel(db.userDao().getUserFromUsername(username.getText().toString()).getId(),"Favorite",R.drawable.like);
                        db.mPlaylistDao().insertPlaylist(playlistUser);
                        //For the recognition playlist
